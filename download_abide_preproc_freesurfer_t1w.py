...
def collect_and_download(out_dir, less_than, greater_than, site, sex, diagnosis,freesurfer_flist):
    """
    ...
    for pheno_row in pheno_list[1:]:
        ...
        if greater_than < row_age < less_than:
            ...
            s3_path = '/'.join([s3_prefix, 'Outputs','freesurfer','5.1',filename])
            print('Adding {0} to download queue...'.format(s3_path))
            for item in freesurfer_flist:
                single_fname = s3_path + item
                s3_paths.append(single_fname)
        else:
            continue
...
if __name__ == '__main__':
    ...
    # read the Freesurfer file list
    with open("freesurfer_files.json","r") as f:
        fs_files = json.load(f)

    fs_flist = []
    for i in fs_files["label"]:
        fs_flist.append('/label/'+i)
    for i in fs_files["mri"]:
        fs_flist.append('/mri/'+i)
    for i in fs_files["scripts"]:
        fs_flist.append('/scripts/'+i)
    for i in fs_files["stats"]:
        fs_flist.append('/stats/'+i)
    for i in fs_files["surf"]:
        fs_flist.append('/surf/'+i)

    # Call the collect and download routine
    collect_and_download(download_data_dir, desired_age_max,
                         desired_age_min, desired_site, desired_sex, desired_diagnosis, fs_flist)
